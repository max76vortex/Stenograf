---
name: workspace-activity-report
description: >-
...
1. **`memory-bank/activity-journal.md`** — строки с локальным ISO-временем и меткой `[Manual]` / `[GitCommit]` / `[Scheduled]`.
2. **`git log`** (если репозиторий инициализирован) — объективное время коммитов.
3. **`memory-bank/progress.md`** — смысловые вехи (даты в тексте могут отличаться от реального времени работ).
